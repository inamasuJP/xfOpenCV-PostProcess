...
int initializeEGL(Display *xdisp, Window &xwindow, EGLDisplay &display, EGLContext &context, EGLSurface &surface,bool verbose)
{
    ...
    if(verbose){
	    ...
	    std::cout << "    Version: " << eglQueryString(display,	EGL_VERSION) << std::endl;
	    std::cout << "    Support API: " << eglQueryString(display,EGL_CLIENT_APIS) << std::endl;
        std::cout << "    Extensions: " << eglQueryString(display,EGL_EXTENSIONS)
		    << std::endl;
    }
    return 0;
}
